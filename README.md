...
- 🧠 **AI Agent 化与 RAG 增强**: 
  - 引入 RAG (检索增强生成) 技术，实现对本地存档内容的深度语义搜索与问答。
  - 引入 Agent 智能体，根据用户定制的喜好（Persona），自动全网巡逻发现高价值信息，并自主判断是否存档与推送。
- 🔄 **多源自动同步**: 
  - 支持绑定知乎、B站、微博等平台的个人账号，自动后台同步“收藏夹”与“关注更新”，实现静默全量存档。
- 📰 **RSS/Atom 订阅集成**: 
  - 原生支持 RSS 订阅源，自动抓取文章全文并生成 AI 摘要，统一入库管理。
- 🤖 **Telegram/社群深度集成**: 
  - 支持将 Bot 拉入任意 Channel 或群组。
  - **全量存档模式**: 自动保存群内所有链接与媒体。
  - **裁判模式**: 让大模型充当“内容裁判”，自动分析群聊内容质量，仅存档高价值信息。
...
| **ffmpeg** | 最新版 | (可选) 用于加快 GIF 转码速度 (25x 快)，安装脚本会尝试自动安装 |
...
- ✅ 私有存档隔离: `contents.archive_metadata.archive` 仅内部使用
