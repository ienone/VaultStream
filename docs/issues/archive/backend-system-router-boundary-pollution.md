...
2026-09-06 已完成。所有外部调用、后台任务调度、任务账本与跨域状态聚合均由对应 service 承担；最后残留的 AI capability 配置、语义索引和连通性投影也进入 `AIDiagnosticsService`。`system.py` 只保留 HTTP contract、依赖注入、错误映射，以及 dashboard、tags、settings 等本模块的短查询。
...
- AI 连通性、模型发现和平台解析测试已下沉到 `AIDiagnosticsService`；模型、embedding、GenAI、HTTP provider 和平台 adapter 调用不再由 router 执行。
- 收藏同步状态、策略检查、预览、触发和三类重试已下沉到 `FavoritesSyncService`；内容导入、run 账本和后台协程创建不再由 router 执行。
- 平台 Cookie、浏览器认证、收藏认证和保活状态聚合已下沉到 `PlatformHealthService`。
- 后台任务汇总、失败详情、run 查询、provider 状态和 Prometheus 文本已下沉到 `SystemDiagnosticsService`；根 `/health` 与 `/api/v1/health` 复用同一可注入 service。
- AI capability 的配置读取、语义索引状态、最近连通性结果和用户可见能力投影已下沉到 `AIDiagnosticsService`。
- `system.py` 从约 1900 行降至约 600 行，并增加结构回归，禁止模型构造、平台 adapter、内容写入、认证探针、后台任务调度、任务账本和 AI capability 聚合重新进入该 router。
- 原 URL、状态码和命名响应 contract 保持不变；API 聚焦回归覆盖系统设置、平台健康、收藏同步和策略路径。

## 关联代码

- `backend/app/routers/system.py`
- `backend/app/services/ai_diagnostics.py`
- `backend/app/services/favorites_sync_service.py`
- `backend/app/services/platform_health_service.py`
- `backend/app/services/system_diagnostics_service.py`
...
- 结构回归：`backend/tests/test_system_router_boundaries.py`。
...
- dashboard、tags 与 settings 是 system/config 模块自身的只读或短写入 contract，没有再次引入跨域副作用；为缩短单文件继续拆同一 URL router 不产生额外 owner、策略或测试收益，因此不作为本问题的未完成项。
- URL、状态码和响应字段保持不变；AI capability API 测试继续覆盖配置组合、语义索引状态和最近连通性投影。
