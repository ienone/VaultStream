...
| Distribution 决策入口 | 已修 | 新增 `DistributionService` 作为规则匹配、自动审批、刷新和入队的业务入口；`engine`/`scheduler` 仅保留兼容包装；API、post-ingest、Agent push 已改用 service；分发回归测试通过 | 后续可在下一次破坏性版本删除兼容包装 |
