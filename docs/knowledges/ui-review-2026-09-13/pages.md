...
每组最多四张截图，通常每行左侧桌面、右侧手机；特殊尺寸写在图内。编号是展示图组，不是路由数。拼图保留截图原始分辨率，可打开查看。[返回主报告](README.md) · [路由与状态映射](coverage.md)
...
**判断与边界：** 聚合数据按真实 blocks/sub_item 结构填充后复拍；初始无子项空态来自样例数据不足，不算渲染故障。
