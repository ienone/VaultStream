...
素材只保留正文使用的对照图、过程拼图和录像；重复原图、独立帧、无消费者的 JSON 索引与浏览器快照不入库。路由和状态映射见覆盖矩阵，尺寸标在图内。
...
审校时 Flutter Web release 构建成功；仅保留审校文档、拼图和录像，不添加正式测试或修改运行代码。图片解码和本地链接已核对。一次性取证脚本已删除，独立服务已停止；本次取证没有外发或部署。
