...
- 错误代码独立存在时也保留技术详情入口；展开状态跨窗口变化保留，普通动画使用共享时序，减少动画直接到终态。
- 业务结果与运行信息使用连续阅读区，字段随字号调整列数；后端 negative/positive tone 用于数值强调。关联对象只省略已由明确动作指向的同一地址，其他关联与领域动作保持原 contract。
