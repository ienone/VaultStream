...
- Section Shell 分区：账号与平台、AI 与发现、推送与通知、外观与系统。
- 主要文件：`features/settings/presentation/tabs/*`

## 当前界面内容

- 页面整体是 `Scaffold` + 毛玻璃 `FrostedAppBar`。页面支持通过 `tab` query 参数打开指定设置分区，例如 `accounts`、`automation`、`push`、`system`；兼容旧 `connection`、`account`、`ai`、`discovery`、`sources`、`bot`、`notifications`、`appearance` 等别名。Debug 模式下 AppBar 右侧提供进入引导页的按钮。
- 桌面端使用 Section Shell：左侧约 292px 的设置分区列表，右侧是当前分区标题头和圆角内容 surface。左侧列表显示分区标题、说明、图标和选中状态；右侧内容区域用 `ConstrainedBox(maxWidth: 1120)` 限制过宽阅读，并用圆角 surface 承载原设置内容。
- 移动端先显示设置分区列表；点击分区后进入全屏详情，AppBar 标题切换为当前分区并显示返回按钮。返回后回到设置分区列表并清除 `tab` query。
...
- 账号与平台健康区域展示平台健康只读摘要。平台摘要以 `SettingGroup` 逐平台列出健康状态、Cookie 状态和收藏同步状态；旧 `/accounts` 路由已删除，不再保留兼容跳转，也不在设置页内保留自跳转入口。
- 当前账号与平台分区只保留平台健康只读摘要，不再保留旧账号中心的连接、检测、解绑、扫码登录、刷新指纹和轻量同步预览按钮；这些主流程后续需要按新分区职责重新实现。
...
- 账号与平台分区承担账号相关配置入口；旧账号中心页面代码已删除，后续账号连接/检测/解绑主流程需要按新分区职责重新实现，而不是恢复旧页面。
- 不展示完整任务运行记录。
- 不承载复杂收藏同步工作流。
- 不应把未实现策略占位暴露为可配置项。
...
- 无本页单独计划；设置页 Section Shell 已落地，后续按新分区职责重建账号连接/检测/解绑主流程，并清理收藏同步 placeholder 策略暴露。目标边界以 `../../plans/2026-06-10-frontend-information-architecture-redesign.plan.md` 和 `../../issues/frontend-favorites-sync-placeholder-strategy-leak.md` 为准。
