# VaultStream 架构设计文档

> **📌 重要说明**: 本文档描述的是原始的生产级架构设计。**当前实现已迁移至轻量化架构**（SQLite + 本地存储 + 任务表队列）。
> 
> 文档中提到的PostgreSQL/Redis/MinIO已被移除，但代码保留了适配器抽象层以便未来扩展。
> 
> **当前架构文档**: [轻量化部署指南](LIGHTWEIGHT_DEPLOYMENT.md)
>
> 以下内容作为设计参考保留。

## 1. 系统概览

VaultStream 是一个多平台内容聚合和归档系统，支持从 B 站等平台抓取内容，存储到私有归档，并可推送到 Telegram 等目标平台。

### 核心特性（当前实现）

- 🎯 多平台支持：当前支持 B 站（视频/动态/专栏）、Twitter/X、微博、知乎、小红书，可扩展其他平台
- 📦 私有归档：完整保存内容元数据和媒体资源（SQLite存储）
- 🖼️ 媒体处理：自动下载图片并转码为 WebP 格式优化存储
- 🔄 异步任务队列：基于 SQLite Task表的任务队列系统
- 🤖 Bot 集成：Telegram Bot 推送到频道
- 💾 本地存储：SHA256内容寻址 + 2级目录分片

---

## 2. 整体架构图

```
┌─────────────────────────────────────────────────────────────────────┐
│                           VaultStream 系统                           │
└─────────────────────────────────────────────────────────────────────┘

┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐
│   用户/客户端     │         │   Telegram Bot   │         │   外部平台 API    │
│                  │         │                  │         │  (Bilibili etc)  │
│  - Web 界面      │         │  - 接收命令      │         │                  │
│  - API 调用      │         │  - 推送到频道    │         │  - 获取内容      │
│  - 直接URL分享   │         │                  │         │  - 下载媒体      │
└────────┬─────────┘         └────────┬─────────┘         └────────┬─────────┘
         │                            │                            │
         │ HTTP API                   │ HTTP API                   │ HTTP/HTTPS
         ▼                            ▼                            ▲
┌─────────────────────────────────────────────────────────────────────┐
│                         FastAPI 应用层                               │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │  API Routes (app/api.py)                                     │  │
│  │  - POST /api/v1/shares         创建分享                       │  │
│  │  - GET  /api/v1/contents       查询内容                       │  │
│  │  - GET  /api/v1/distribution-queue/items  查询队列项          │  │
│  │  - POST /api/v1/distribution-queue/items/{id}/push-now 推送   │  │
│  │  - GET  /api/v1/health         健康检查                       │  │
│  └──────────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │  Middleware                                                   │  │
│  │  - Request ID 追踪                                            │  │
│  │  - CORS 跨域                                                  │  │
│  │  - API Token 鉴权                                             │  │
│  └──────────────────────────────────────────────────────────────┘  │
└───────────┬───────────────────────┬──────────────────────────────────┘
            │                       │
            │                       │
            ▼                       ▼
┌───────────────────────┐   ┌──────────────────────────┐
│   任务队列层           │   │   适配器层                │
│   (app/queue.py)      │   │   (app/adapters/)        │
│                       │   │                          │
│  ┌─────────────────┐  │   │  ┌────────────────────┐  │
│  │  TaskQueue      │  │   │  │  AdapterFactory    │  │
│  │  - enqueue()    │  │   │  │  - detect_platform │  │
│  │  - dequeue()    │  │   │  │  - create()        │  │
│  │  - mark_done()  │  │   │  └────────┬───────────┘  │
│  └────────┬────────┘  │   │           │              │
│           │           │   │           ▼              │
│           │ Redis     │   │  ┌────────────────────┐  │
│           │ LPUSH/    │   │  │  BilibiliAdapter   │  │
│           │ BRPOP     │   │  │  - parse()         │  │
│           ▼           │   │  │  - clean_url()     │  │
│  ┌─────────────────┐  │   │  │  - fetch_opus()    │  │
│  │  Redis          │  │   │  │  - fetch_video()   │  │
│  │  - tasks queue  │  │   │  └────────────────────┘  │
│  │  - dead letters │  │   │                          │
│  └─────────────────┘  │   │  (可扩展其他平台适配器)   │
└───────────────────────┘   └────────────┬─────────────┘
                                         │
            ┌────────────────────────────┤
            │                            │
            ▼                            ▼
┌───────────────────────┐   ┌──────────────────────────┐
│    后台任务处理器        │   │   媒体处理层              │
│   (app/worker.py)     │   │   (app/media_processing) │
│                       │   │                          │
│  ┌─────────────────┐  │   │  ┌────────────────────┐  │
│  │  TaskWorker     │  │   │  │   图片下载和转码     │  │
│  │  - start()      │  │   │  │  - download_image  │  │
│  │  - process()    │  │   │  │  - to_webp()       │  │
│  │  - retry logic  │  │   │  │  - content hash    │  │
│  └────────┬────────┘  │   │  └────────────────────┘  │
│           │           │   │                          │
│           │  轮询任务   │   │  (未来: 音视频转码)       │
│           ▼           │   └────────────┬─────────────┘
│  持续从队列获取任务    │                │
└───────────┬───────────┘                │
            │                            │
            ▼                            ▼
┌─────────────────────────────────────────────────────────────────────┐
│                          数据持久层                                   │
│                                                                     │
│  ┌────────────────────────┐         ┌────────────────────────────┐  │
│  │  PostgreSQL 数据库      │         │  对象存储后端                │  │
│  │  (app/database.py)     │         │  (app/storage.py)          │  │
│  │                        │         │                            │  │
│  │  ┌──────────────────┐  │         │  ┌──────────────────────┐  │  │
│  │  │  Contents 表     │  │         │  │  LocalStorage        │  │  │
│  │  │  - id            │  │         │  │  - 本地文件系统        │  │  │
│  │  │  - platform      │  │         │  │  - data/storage/     │  │  │
│  │  │  - url           │  │         │  └──────────────────────┘  │  │
│  │  │  - status        │  │         │           或               │  │
│  │  │  - title         │  │         │  ┌──────────────────────┐  │  │
│  │  │  - metadata      │  │         │  │  S3Storage (MinIO)   │  │  │
│  │  │  - cover_url     │  │         │  │  - endpoint          │  │  │
│  │  │  - media_urls    │  │         │  │  - bucket            │  │  │
│  │  │  - raw_metadata  │  │         │  │  - presigned URLs    │  │  │
│  │  └──────────────────┘  │         │  └──────────────────────┘  │  │
│  │                        │         │                            │  │
│  │  ┌──────────────────┐  │         │  存储结构 (内容寻址):         │  │
│  │  │  ContentSources  │  │         │  blobs/sha256/XX/YY/       │  │
│  │  │  - source info   │  │         │    {sha256}.webp           │  │
│  │  └──────────────────┘  │         │                            │  │
│  │                        │         └────────────────────────────┘  │
│  │  ┌──────────────────┐  │                                         │
│  │  │  PushedRecords   │  │                                         │
│  │  │  - push history  │  │                                         │
│  │  └──────────────────┘  │                                         │
│  └────────────────────────┘                                         │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 3. 核心组件详解

### 3.1 FastAPI 应用层 (`app/main.py`, `app/api.py`)

职责：
- HTTP API 入口
- 请求路由和参数验证
- 中间件处理（日志、鉴权、CORS）

主要端点：

| 端点 | 方法 | 功能 | 备注 |
|------|------|------|------|
| `/api/v1/shares` | POST | 创建内容分享 | 接收URL，识别平台，入库并入队 |
| `/api/v1/contents` | GET | 查询内容列表 | 支持过滤、分页 |
| `/api/v1/contents/{id}` | GET | 获取内容详情 | 包含完整元数据 |
| `/api/v1/distribution-queue/items/{item_id}/push-now` | POST | 立即执行单个队列项推送 | 复用队列 Worker 逻辑 |
| `/api/v1/health` | GET | 健康检查 | 返回系统状态 |

### 3.2 适配器层 (`app/adapters/`)

设计模式：工厂模式 + 策略模式

```python
AdapterFactory
├── detect_platform(url) → Platform
├── create(platform) → BaseAdapter
│
BaseAdapter (抽象基类)
├── parse(url) → ParsedContent
├── clean_url(url) → str
│
BilibiliAdapter (具体实现)
├── parse(url)
│   ├── 识别内容类型 (opus/video/article)
│   ├── 调用对应解析器
│   └── 返回统一结构
├── fetch_opus()
├── fetch_video()
└── fetch_article()
```

扩展性：
- 新增平台只需继承 `BaseAdapter`
- 实现 `parse()` 和 `clean_url()` 方法
- 在 `AdapterFactory` 注册

### 3.3 任务队列 (`app/queue.py`)

实现：基于 Redis List 的简单队列

```python
TaskQueue
├── enqueue(task_data)        # LPUSH 到队列
├── dequeue(timeout)           # BRPOP 阻塞获取
├── mark_complete(content_id)  # 从 processing 集合移除
└── push_dead_letter(task)     # 失败任务归档
```

任务结构：
```json
{
  "schema_version": 1,
  "action": "parse",
  "content_id": 123,
  "task_id": "uuid",
  "attempt": 0,
  "max_attempts": 3
}
```

### 3.4 后台 Worker (`app/worker.py`)

核心流程：

```python
TaskWorker.start()
  └─> while running:
        ├─> task = queue.dequeue(timeout=5)
        ├─> if task:
        │     └─> process_task(task)
        │           ├─> 获取 Content 记录
        │           ├─> 更新状态为 PROCESSING
        │           ├─> 调用 Adapter.parse()
        │           ├─> 如果启用媒体处理:
        │           │     └─> 处理归档图片
        │           ├─> 更新 Content 信息
        │           ├─> 状态改为 PULLED
        │           └─> mark_complete()
        └─> 异常处理：
              ├─> 记录失败信息
              ├─> 重试逻辑（指数退避）
              └─> 达到最大次数 → dead letter
```

重试策略：
- 可重试错误：网络超时、临时故障
- 不可重试错误：权限不足、内容不存在
- 指数退避：1s → 2s → 4s → 8s...

### 3.5 媒体处理 (`app/media_processing.py`)

当前实现：归档图片处理

```python
store_archive_images_as_webp()
  ├─> 遍历 archive.images
  ├─> 对每张图片:
  │     ├─> 下载原图 (httpx)
  │     ├─> 转码为 WebP (Pillow)
  │     ├─> 计算 SHA256
  │     ├─> 生成内容寻址 key
  │     ├─> 上传到存储后端
  │     └─> 更新 metadata (stored_key, stored_url)
  └─> 返回更新后的 archive
```

内容寻址：
```
key = blobs/sha256/AB/CD/ABCD1234...WXYZ.webp
      ↑       ↑    ↑  ↑   ↑
      |       |    |  |   完整 SHA256
      |       |    |  SHA256[2:4]
      |       |    SHA256[:2]
      |       哈希算法
      blob 类型
```

优点：
- 去重：相同内容只存储一次
- 可验证：可通过 SHA256 校验完整性
- 分布均匀：前缀分桶避免单目录文件过多

### 3.6 存储后端 (`app/storage.py`)

抽象接口：

```python
StorageBackend (抽象基类)
├── put_bytes(key, data, content_type) → StoredObject
├── exists(key) → bool
└── get_url(key) → Optional[str]
```

两种实现：

#### LocalStorageBackend
- 存储位置：`data/storage/` (可配置)
- URL 生成：`{public_base_url}/{key}`
- 适用场景：开发环境、小规模部署

#### S3StorageBackend (MinIO 兼容)
- 客户端：boto3
- 寻址方式：Path-style (兼容 MinIO)
- URL 生成：
  - 公开桶：`{public_base_url}/{bucket}/{key}`
  - 私有桶：预签名 URL (presigned)
- 适用场景：生产环境、大规模部署

MinIO 集成要点：
```python
# 自动创建 bucket
await storage.ensure_bucket()

# 预签名 URL (私有桶访问)
storage_s3_presign_urls = True
storage_s3_presign_expires = 3600  # 1小时有效期
```

### 3.7 数据库模型 (`app/models.py`)

核心表结构：

```sql
-- 内容表
CREATE TABLE contents (
    id SERIAL PRIMARY KEY,
    platform VARCHAR NOT NULL,           -- BILIBILI, YOUTUBE, etc
    url VARCHAR NOT NULL,                -- 原始 URL
    canonical_url VARCHAR NOT NULL,      -- 规范化 URL (去重用)
    clean_url VARCHAR,                   -- 清理后 URL (展示用)
    status VARCHAR NOT NULL,             -- UNPROCESSED, PROCESSING, PULLED, FAILED
    
    -- 元数据
    title VARCHAR,
    description TEXT,
    author_name VARCHAR,
    author_id VARCHAR,
    platform_id VARCHAR,                 -- 平台内容 ID (BV号等)
    content_type VARCHAR,                -- opus, video, article
    
    -- 媒体资源 (对外分享)
    cover_url VARCHAR,                   -- 封面图 URL
    media_urls JSON,                     -- 媒体 URL 列表
    
    -- 私有归档
    raw_metadata JSON,                   -- 完整元数据 (含 archive)
    
    -- 统计数据
    view_count INTEGER,
    like_count INTEGER,
    collect_count INTEGER,
    share_count INTEGER,
    comment_count INTEGER,
    extra_stats JSON,                    -- 平台特有统计
    
    -- 分类和来源
    tags JSON,                           -- 标签列表
    source VARCHAR,                      -- 来源标识
    is_nsfw BOOLEAN,
    
    -- 错误处理
    failure_count INTEGER DEFAULT 0,
    last_error TEXT,
    last_error_type VARCHAR,
    last_error_detail JSON,
    last_error_at TIMESTAMP,
    
    -- 时间戳
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    published_at TIMESTAMP,              -- 内容发布时间
    
    UNIQUE(platform, canonical_url)      -- 组合唯一约束
);

-- 内容来源表
CREATE TABLE content_sources (
    id SERIAL PRIMARY KEY,
    content_id INTEGER REFERENCES contents(id),
    source VARCHAR,                      -- 来源渠道
    tags_snapshot JSON,                  -- 创建时的标签
    note TEXT,                           -- 备注
    client_context JSON,                 -- 客户端上下文
    created_at TIMESTAMP DEFAULT NOW()
);

-- 推送记录表
CREATE TABLE pushed_records (
    id SERIAL PRIMARY KEY,
    content_id INTEGER REFERENCES contents(id),
    target_platform VARCHAR NOT NULL,    -- TG_CHANNEL_xxx
    pushed_at TIMESTAMP DEFAULT NOW(),
    push_context JSON                    -- 推送上下文
);
```

---

## 4. 数据流和工作流程

### 4.1 内容创建流程

```
用户提交 URL
   │
   ├─> POST /api/v1/shares
   │
   ├─> [API] 预处理 URL
   │     ├─> normalize_bilibili_url() (支持 BV/av/cv)
   │     └─> canonicalize_url()
   │
   ├─> [API] 检测平台
   │     └─> AdapterFactory.detect_platform()
   │
   ├─> [API] 计算 canonical_url (去重)
   │     └─> adapter.clean_url()
   │
   ├─> [DB] 检查是否已存在
   │     └─> SELECT WHERE platform + canonical_url
   │
   ├─> 如果不存在:
   │     ├─> 创建 Content 记录 (status=UNPROCESSED)
   │     ├─> 创建 ContentSource 记录
   │     ├─> 提交解析任务到队列
   │     │     └─> task_queue.enqueue({content_id, action: "parse"})
   │     └─> 返回 content_id
   │
   └─> 如果已存在:
         ├─> 仍创建 ContentSource (记录来源)
         └─> 返回已有 content_id
```

### 4.2 内容解析流程 (Worker)

```
Worker 轮询队列
   │
   ├─> task = queue.dequeue(timeout=5)
   │
   ├─> 获取 Content 记录
   │     └─> SELECT FROM contents WHERE id = task.content_id
   │
   ├─> 更新状态 → PROCESSING
   │
   ├─> 调用适配器解析
   │     ├─> adapter = AdapterFactory.create(platform, cookies)
   │     ├─> parsed = adapter.parse(url)
   │     │     │
   │     │     ├─> [Bilibili] 识别类型
   │     │     │     ├─> opus → fetch_opus()
   │     │     │     ├─> video → fetch_video()
   │     │     │     └─> article → fetch_article()
   │     │     │
   │     │     ├─> 调用平台 API
   │     │     │     └─> httpx.get(api_url, cookies, headers)
   │     │     │
   │     │     └─> 返回 ParsedContent
   │     │           ├─> clean_url
   │     │           ├─> content_type
   │     │           ├─> title, description
   │     │           ├─> author_name, author_id
   │     │           ├─> cover_url, media_urls
   │     │           ├─> stats (view, like, etc)
   │     │           └─> raw_metadata (完整数据)
   │     │
   │     └─> 重试逻辑 (最多3次)
   │
   ├─> 如果启用媒体处理 (enable_archive_media_processing=True):
   │     │
   │     ├─> 检查 raw_metadata.archive.images
   │     │
   │     ├─> store_archive_images_as_webp()
   │     │     │
   │     │     ├─> 对每张图片:
   │     │     │     ├─> 下载原图
   │     │     │     │     └─> httpx.get(img_url, headers)
   │     │     │     │
   │     │     │     ├─> WebP 转码
   │     │     │     │     ├─> PIL.Image.open(data)
   │     │     │     │     └─> save(format="WEBP", quality=80)
   │     │     │     │
   │     │     │     ├─> 计算 SHA256
   │     │     │     │     └─> hashlib.sha256(webp_data).hexdigest()
   │     │     │     │
   │     │     │     ├─> 生成存储 key
   │     │     │     │     └─> blobs/sha256/XX/YY/{hash}.webp
   │     │     │     │
   │     │     │     ├─> 上传到存储后端
   │     │     │     │     ├─> storage.exists(key) ? 跳过
   │     │     │     │     └─> storage.put_bytes(key, data)
   │     │     │     │
   │     │     │     └─> 更新 metadata
   │     │     │           ├─> stored_key
   │     │     │           ├─> stored_url (presigned or public)
   │     │     │           ├─> stored_sha256
   │     │     │           └─> width, height, size
   │     │     │
   │     │     └─> 返回更新后的 metadata
   │     │
   │     └─> 更新 Content.raw_metadata
   │
   ├─> 更新 Content 字段
   │     ├─> title, description
   │     ├─> author_name, author_id
   │     ├─> platform_id
   │     ├─> content_type
   │     ├─> cover_url, media_urls (对外分享)
   │     ├─> view_count, like_count, etc
   │     ├─> extra_stats
   │     ├─> raw_metadata (完整归档)
   │     └─> published_at
   │
   ├─> 更新状态 → PULLED
   │     └─> 清理 last_error, last_error_type 等
   │
   ├─> 提交事务
   │
   └─> queue.mark_complete(content_id)
```

### 4.3 Bot 推送流程

```
用户发送 /get [tag]
   │
   ├─> Bot 收到命令
   │
   ├─> GET /api/v1/bot/chats/{chat_id}
   ├─> GET /api/v1/distribution-queue/items?status=scheduled&bot_chat_id=...&size=50
   ├─> 读取内容详情并按 tag/platform 筛选
   ├─> POST /api/v1/distribution-queue/items/{item_id}/push-now
   │     └─> [Worker] 执行推送并写入 pushed_records
   │
   └─> 回复用户: ✅ 已发送
```

### 4.4 MinIO 存储流程

```
Worker 处理归档图片
   │
   ├─> storage = get_storage_backend()
   │     └─> 根据 storage_backend 配置
   │           ├─> "local" → LocalStorageBackend
   │           └─> "s3" → S3StorageBackend
   │
   ├─> [MinIO] 确保 bucket 存在
   │     └─> storage.ensure_bucket()
   │           ├─> s3.head_bucket(bucket)
   │           │     └─> 404 → 创建 bucket
   │           └─> s3.create_bucket(bucket)
   │
   ├─> 下载并转码图片 → webp_data
   │
   ├─> 计算内容寻址 key
   │     └─> blobs/sha256/AB/CD/ABCD...WXYZ.webp
   │
   ├─> 检查是否已存在
   │     └─> exists = storage.exists(key)
   │           └─> s3.head_object(bucket, key)
   │
   ├─> 如果不存在:
   │     └─> storage.put_bytes(key, webp_data, "image/webp")
   │           ├─> s3.put_object(
   │           │     Bucket=bucket,
   │           │     Key=key,
   │           │     Body=webp_data,
   │           │     ContentType="image/webp"
   │           │   )
   │           │
   │           └─> 返回 StoredObject
   │                 ├─> key
   │                 ├─> size
   │                 ├─> sha256
   │                 └─> url
   │
   ├─> 获取访问 URL
   │     └─> url = storage.get_url(key)
   │           │
   │           ├─> 如果启用预签名:
   │           │     └─> s3.generate_presigned_url(
   │           │           "get_object",
   │           │           {Bucket, Key},
   │           │           ExpiresIn=3600
   │           │         )
   │           │
   │           └─> 否则 (公开桶):
   │                 └─> {public_base_url}/{bucket}/{key}
   │
   └─> 保存到 metadata
         └─> archive.images[i].stored_url = url
```

---

## 5. 配置说明

### 5.1 环境变量 (.env)

```bash
# === 基础配置 ===
APP_ENV=dev                              # 环境: dev/prod
DEBUG=true                               # 调试模式
DEBUG_SQL=false                          # SQL 日志 (新增)
LOG_LEVEL=INFO                           # 日志级别
LOG_FORMAT=json                          # 日志格式: json/text

# === 数据库 ===
DATABASE_URL=postgresql+asyncpg://user:password@localhost:5432/vaultstream

# === Redis ===
REDIS_URL=redis://localhost:6379/0

# === API ===
API_HOST=0.0.0.0
API_PORT=8000
API_TOKEN=your_secret_token              # API 鉴权 Token

# === Telegram Bot ===
ENABLE_BOT=false                         # 是否启用 Bot
TELEGRAM_BOT_TOKEN=123456:ABC-DEF...     # Bot Token
TELEGRAM_CHANNEL_ID=@your_channel        # 频道 ID

# === 全局HTTP代理（可选，用于Telegram、Twitter、YouTube等）===
HTTP_PROXY=http://127.0.0.1:7890         # HTTP代理地址
# HTTPS_PROXY=http://127.0.0.1:7890      # HTTPS代理地址（可选）

# === B站 Cookies (可选) ===
BILIBILI_SESSDATA=your_sessdata
BILIBILI_BILI_JCT=your_bili_jct
BILIBILI_BUVID3=your_buvid3

# === 存储后端 ===
STORAGE_BACKEND=s3                       # local 或 s3
STORAGE_PUBLIC_BASE_URL=http://localhost:9000  # 公开访问地址

# --- 本地存储 ---
STORAGE_LOCAL_ROOT=data/storage

# --- MinIO/S3 存储 ---
STORAGE_S3_ENDPOINT=http://localhost:9000
STORAGE_S3_REGION=us-east-1
STORAGE_S3_BUCKET=vaultstream
STORAGE_S3_ACCESS_KEY=minioadmin
STORAGE_S3_SECRET_KEY=minioadmin
STORAGE_S3_PRESIGN_URLS=true             # 使用预签名 URL
STORAGE_S3_PRESIGN_EXPIRES=3600          # 预签名有效期(秒)

# === 媒体处理 ===
ENABLE_ARCHIVE_MEDIA_PROCESSING=true     # 启用归档媒体处理
ARCHIVE_IMAGE_WEBP_QUALITY=80            # WebP 质量 (1-100)
ARCHIVE_IMAGE_MAX_COUNT=20               # 最大图片数量
```

### 5.2 Docker Compose 服务

```yaml
services:
  postgres:
    image: postgres:15
    environment:
      POSTGRES_DB: vaultstream
      POSTGRES_USER: user
      POSTGRES_PASSWORD: password
    ports:
      - "5432:5432"
    volumes:
      - postgres_data:/var/lib/postgresql/data

  redis:
    image: redis:7-alpine
    ports:
      - "6379:6379"

  minio:
    image: minio/minio
    command: server /data --console-address ":9001"
    environment:
      MINIO_ROOT_USER: minioadmin
      MINIO_ROOT_PASSWORD: minioadmin
    ports:
      - "9000:9000"       # API
      - "9001:9001"       # Console
    volumes:
      - minio_data:/data
```

---

## 6. 状态机和错误处理

### 6.1 Content 状态转换

```
UNPROCESSED  ──┐
               │ Worker 获取任务
               ↓
           PROCESSING ──┐
               │        │ 解析失败
               │        ↓
               │     FAILED ──┐
               │        ↑     │ 重试
               │        └─────┘ (最多3次)
               │ 解析成功
               ↓
            PULLED
```

### 6.2 错误分类

| 错误类型 | 是否重试 | 处理策略 |
|---------|---------|---------|
| 网络超时 | ✅ | 指数退避重试 |
| 临时故障(5xx) | ✅ | 指数退避重试 |
| 权限不足(403) | ❌ | 标记失败，需人工介入 |
| 内容不存在(404) | ❌ | 标记失败 |
| 内容需登录 | ❌ | 记录 auth_required |
| 解析异常 | ❌ | 记录详细错误信息 |

### 6.3 失败记录

```python
Content.failure_count       # 失败次数统计
Content.last_error          # 错误消息
Content.last_error_type     # 错误类型 (异常类名)
Content.last_error_detail   # 详细信息 (含 traceback)
Content.last_error_at       # 失败时间
```

---

## 7. 性能优化

### 7.1 已实施优化

1. 异步 I/O
   - FastAPI 异步路由
   - asyncpg 异步数据库
   - httpx 异步 HTTP 客户端

2. 连接复用
   - 数据库连接池
   - Bot httpx client 复用
   - Redis 连接复用

3. 任务队列
   - 解耦请求和处理
   - 后台异步处理
   - 并发控制

4. 内容寻址存储
   - 去重存储
   - 减少网络传输
   - 降低存储成本

5. 日志精简 (本次优化)
   - 关闭 SQL echo
   - 减少 debug 日志
   - 独立 debug_sql 配置

6. Bot 优化 (本次优化)
   - 复用 httpx 客户端
   - 异步标记推送状态
   - 减少阻塞等待

### 7.2 未来优化方向

1. 缓存层
   - Redis 缓存热点内容
   - CDN 加速媒体访问

2. 批量处理
   - 批量下载图片
   - 批量数据库操作

3. 并发控制
   - Worker 多进程/多线程
   - 限流和熔断

4. 监控和告警
   - Prometheus + Grafana
   - 错误率、延迟监控

---

## 8. 安全考虑

### 8.1 API 鉴权

- 简单 Token 鉴权 (M1)
- 支持 `X-API-Token` 头或 `Authorization: Bearer`
- 未配置时放行（便于开发）

### 8.2 数据隔离

- MinIO 私有桶 + 预签名 URL
- 敏感配置使用 `SecretStr`
- 日志不输出敏感值

### 8.3 输入验证

- Pydantic 模型验证
- URL 规范化防注入
- 平台检测防滥用

---

## 9. 扩展性

### 9.1 新增平台适配器

```python
# 1. 创建适配器
class YouTubeAdapter(BaseAdapter):
    platform = Platform.YOUTUBE
    
    async def parse(self, url: str) -> ParsedContent:
        # 实现解析逻辑
        pass
    
    async def clean_url(self, url: str) -> str:
        # URL 规范化
        pass

# 2. 注册到工厂
AdapterFactory._adapters[Platform.YOUTUBE] = YouTubeAdapter

# 3. 添加平台检测规则
AdapterFactory._patterns.append(
    (re.compile(r'youtube\.com|youtu\.be'), Platform.YOUTUBE)
)
```

### 9.2 新增存储后端

```python
class AzureBlobStorage(StorageBackend):
    async def put_bytes(self, *, key, data, content_type):
        # 实现上传
        pass
    
    async def exists(self, *, key):
        # 检查存在
        pass
    
    def get_url(self, *, key):
        # 生成访问 URL
        pass
```

### 9.3 新增推送目标

```python
# 添加推送记录时使用不同 target_platform
# 例如: DISCORD_CHANNEL_xxx, SLACK_CHANNEL_xxx
```

---

## 10. 部署架构

### 开发环境

```
MacBook/PC
├─> Docker Desktop
│   ├─> PostgreSQL (5432)
│   ├─> Redis (6379)
│   └─> MinIO (9000, 9001)
├─> Python venv
│   ├─> FastAPI App (8000)
│   └─> Worker (background)
└─> Telegram Bot (可选)
```

### 生产环境 (推荐)

```
┌─────────────────────────────────────────┐
│             反向代理 (Nginx)             │
│  - HTTPS 终止                            │
│  - 静态文件服务                          │
│  - 负载均衡                              │
└──────────┬──────────────────────────────┘
           │
┌──────────▼──────────────────────────────┐
│         应用层 (多实例)                  │
│  ┌─────────────┐  ┌─────────────┐       │
│  │  FastAPI #1 │  │  FastAPI #2 │       │
│  └─────────────┘  └─────────────┘       │
│  ┌─────────────┐  ┌─────────────┐       │
│  │  Worker #1  │  │  Worker #2  │       │
│  └─────────────┘  └─────────────┘       │
│  ┌─────────────┐                        │
│  │     Bot     │                        │
│  └─────────────┘                        │
└───────────┬─────────────────────────────┘
            │
┌───────────▼─────────────────────────────┐
│          数据层 (托管服务)               │
│  ┌──────────────┐  ┌──────────────┐    │
│  │  PostgreSQL  │  │    Redis     │    │
│  │  (RDS/云)    │  │  (云缓存)    │    │
│  └──────────────┘  └──────────────┘    │
│  ┌──────────────┐                      │
│  │    MinIO     │                      │
│  │  (对象存储)  │                      │
│  └──────────────┘                      │
└─────────────────────────────────────────┘
```

---

## 11. 技术栈总结

| 层次 | 技术 | 用途 |
|------|------|------|
| Web 框架 | FastAPI | 异步 API 服务 |
| 数据库 | PostgreSQL | 关系型数据存储 |
| ORM | SQLAlchemy (async) | 数据库访问 |
| 缓存/队列 | Redis | 任务队列、会话存储 |
| 对象存储 | MinIO (S3兼容) | 媒体文件存储 |
| HTTP 客户端 | httpx | 异步 HTTP 请求 |
| Bot 框架 | python-telegram-bot | Telegram 集成 |
| 图像处理 | Pillow | WebP 转码 |
| 日志 | loguru | 结构化日志 |
| 配置管理 | pydantic-settings | 环境变量管理 |
| 容器化 | Docker Compose | 本地开发环境 |

---

## 附录：快速参考

### 启动服务

```bash
# 启动基础设施
docker-compose up -d postgres redis minio

# 启动应用
./start.sh              # FastAPI + Worker

# 启动 Bot (另一个终端)
python -m app.bot
```

### 常用 API

```bash
# 创建分享
curl -X POST http://localhost:8000/api/v1/shares \
  -H "Content-Type: application/json" \
  -d '{"url": "https://www.bilibili.com/opus/933099353259638816"}'

# 查询内容
curl http://localhost:8000/api/v1/contents?status=PULLED

# 健康检查
curl http://localhost:8000/api/v1/health
```

### 调试技巧

```bash
# 开启 SQL 日志
DEBUG_SQL=true ./start.sh

# 查看 Redis 队列
redis-cli
> LLEN vaultstream:tasks
> LRANGE vaultstream:tasks 0 -1

# 查看 MinIO 对象
mc alias set local http://localhost:9000 minioadmin minioadmin
mc ls local/vaultstream/blobs/sha256/
```

---

版本: v0.1  
最后更新: 2026-01-03  
