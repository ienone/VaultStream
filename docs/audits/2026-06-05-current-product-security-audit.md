...
当前已有等价的最近运行记录结构，已覆盖收藏同步、发现源同步和语义索引重建：手动触发会返回 `run_id`，后台诊断响应可列出最近运行状态。后续仍需要把同一模型扩展到解析、摘要、单条自动 Embedding、巡逻评分和分发任务，并提供统一任务详情页。
...
- 扩展统一任务运行记录：当前已覆盖收藏同步、发现同步和语义索引重建，后续继续覆盖解析、摘要、单条自动 Embedding、巡逻评分和分发。
