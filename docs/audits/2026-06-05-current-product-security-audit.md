...
当前已有等价的最近运行记录结构，已覆盖收藏同步、发现源同步、语义索引重建、内容重新解析、手动摘要生成和单条分发队列立即推送：手动触发会返回 `run_id`，后台诊断响应可列出最近运行状态。后续仍需要把同一模型扩展到解析主队列、单条自动 Embedding、巡逻评分、分发批量操作和分发 worker 自动轮询任务，并提供统一任务详情页。
...
- 扩展统一任务运行记录：当前已覆盖收藏同步、发现同步、语义索引重建、内容重新解析、手动摘要生成和单条分发队列立即推送，后续继续覆盖解析主队列、单条自动 Embedding、巡逻评分、分发批量操作和分发 worker 自动轮询任务。
