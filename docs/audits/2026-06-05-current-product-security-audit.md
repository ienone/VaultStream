...
当前已有等价的最近运行记录结构，已覆盖解析主队列、收藏同步、发现源同步、巡逻评分、语义索引重建、单条自动 Embedding、内容重新解析、手动摘要生成、单条分发队列立即推送、content-level/批量分发立即排期，以及分发 worker 自动轮询任务：手动触发会返回 `run_id`，后台诊断响应可列出最近运行状态。动态页后台诊断卡已展示最近运行，并可查看单次 run 的状态、错误、元数据和结果。
...
- 手动触发前已有首版导入预览：后端 `POST /api/v1/favorites-sync/preview` 会只读拉取本轮候选，估算新增/重复/跳过数量，不写入内容且不推进 cursor；前端会在单平台或全平台手动同步前展示预览并要求确认。
...
建议把收藏同步继续升级为独立产品能力，而不是设置页里的一个后台开关：保留当前平台账号健康、运行记录、导入预览和失败 run 重试能力，继续补齐同步范围、失败列表、重复处理策略、取消收藏/删除策略和按失败项重试。
...
- 平台能力测试入口仍是首版，只覆盖登录检测、扫码/重新连接和收藏同步预览；解析测试、推送目标健康和发现源健康尚未纳入同一矩阵。
...
- 后处理状态仍未提供后台任务日志跳转、按单个失败分块重试、巡逻评分手动触发按钮，也尚未区分真实模型连通性失败和仅缺少配置。
...
- 收藏同步已具备运行记录、导入预览和失败 run 重试；后续升级重点是收藏夹/分组范围、取消收藏/删除策略、冲突策略和按失败项重试。
...
- 将 LLM 配置改为能力状态优先：当前已有首版能力摘要和内容后处理链路的配置/修复建议；后续补齐真实连通性测试和更广泛错误联动。
- 内容详情页后处理状态已展示摘要、语义索引、巡逻评分、分发、失败详情、修复建议和重试入口；后续补齐任务日志跳转、单个失败项重试和巡逻评分手动触发。
- 错误提示从底层异常改为用户可行动的分类。
- 新增目标时提供历史内容补建队列选项。
