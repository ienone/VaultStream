...
队列解析的网络抓取和归档媒体准备发生在短写事务之外。网络返回后先取得当前 Task 租约（手动解析则取得内容行）的 SQLite 写锁，再刷新当前 `manual_edit_fields`、人工字段和 `deleted_at`，最后在同一事务写入解析事实、媒体绑定和 Task 完成；因此抓取期间的人工编辑不会被旧快照覆盖，软删或硬删内容安全跳过，迟到 worker 不能提交。普通重复任务遇到已成功内容直接跳过并结算，不在该无保护分支补写归档媒体。摘要、索引和自动审批位于解析提交之后；其失败不回滚正文或把内容改回解析失败，但当前尚不提供跨进程耐久接续。
