...
- 知乎回答/文章、问题/用户主页及带访问 token 的小红书笔记和部分主页支持独立匿名读取；知乎网页使用按需启动的 Chromium，Tardis 可读内容不启动浏览器。已有 Cookie 供账号路径使用，不是所有公开内容的前置要求。公开解析不自动刷新知乎指纹。
- 通用网页先使用明确的文章结构直接提取，无法确定时才调用模型；HTML 获取不重复发起相同 GET。

队列解析的网络抓取和归档媒体准备发生在短写事务之外。网络返回后检查 Task 仍为 RUNNING（手动解析则锁定内容行），刷新当前 `manual_edit_fields`、人工字段和 `deleted_at`，再同事务写入解析事实、媒体绑定和 Task 完成。抓取期间的人工编辑不会被旧快照覆盖，已删除内容安全跳过。已成功内容的普通重复任务直接完成。解析失败不自动重复整个方案；摘要、索引和审批位于解析提交之后，其失败不降级正文，但尚不提供耐久接续。
...
PDF 上传提交原件后调度 document_extract；提取期间不占用上传事务，原文按资产与页码保存，body 仍是用户保存说明。原生文本生成后再按自动索引开关调度 embedding。详情和手动重新提取使用独立 document-text contract。自动摘要移至 PDF 提取后执行，仍受 enable_auto_summary 控制；摘要读取可用原生页，随后建立索引。扫描 OCR 与复杂版面仍未完成，见文档原生文本执行记录。

粉丝、关注、回答数等平台指标只进入 `extra_stats`，不再占用浏览/分享/评论字段；点赞、评论等标准计数不重复保存平台别名。展示用 `clean_url` 由去重身份和可选解析落地地址生成，重复分享不覆盖已解析的落地地址。

### 详情媒体引用（2026-09-26）

详情、重新解析和候选采纳共用一个 presenter，直接从资产原址与存储键映射到 manifest 的精确来源 URL；已删除旧媒体地址中转及从签名 URL 反推存储键的逻辑。卡片和搜索封面、头像也直接选取 manifest，不另拼媒体路径或缩略图参数。签名只存在于响应，不写回正文或数据库；前端只消费返回的来源。

`url` 保留入库身份；`clean_url` 只返回有效的 HTTP(S) 外部原文链接，没有原文时为空。推送和书签展示不得把内部身份作为原文链接。
