...
新解析和发现内容入库的媒体处理读取持久化的归档总开关及图片/视频子开关。`PARSE_SUCCESS` 内容再次进入非 force 的普通解析入口时跳过正文解析，并按归档开关排入独立 `repair_media` 任务。本地失败上报经服务端核验后，存在原始来源时也排入同一持久队列；同内容的待执行/运行中任务合并。修复不重新运行摘要或推送，提交前核对内容版本和任务所有权，期间发生编辑或删除则放弃旧结果。关闭的媒体类型保持原状态。关闭某一媒体类型只跳过后续下载，不删除已归档资产，也不抢占已经开始的单项下载。
...
- [图片归档与缓存残留问题](../../issues/2026-09-19-media-localization-integrity.md)：自动补齐与缓存身份已完成本地验收；生产批量处理与部署待确认目标环境。
