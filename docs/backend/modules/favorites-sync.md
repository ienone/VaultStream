...
平台最近结果和最近同步时间从运行账本生成；每个平台最近结果所属运行不受普通历史数量清理影响，不再把结果列表复制到平台/全局设置。失败只结算失败，不先写成功时间；跳过的调度轮询不产生成功运行。

## Telegram Saved Messages

自动频道同步排除已配置的 Telegram 推送目标，避免推送消息重新入库并转发到自身或其他目标。此限制不影响用户主动保存到 Saved Messages 的消息。

账号任务独立读取 Saved Messages，从最早历史开始按消息水位增量分页；不把私有文本和附件伪造为公开网页地址。与频道中相同内容共用内容、媒体，增加永久收藏来源并转为 promoted、清除 expire_at。已提交条目重复读取幂等，整页成功才推进游标。现阶段不自动删除远端取消收藏的本地存档；历史编辑及删除更新仍待接入，不能称为双向镜像。
