    ...
    async def process_task(self, task_data: dict):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                # 解析内容
                logger.info(f"开始解析内容: {content.url}")
                ...
                await session.commit()
                logger.info(f"内容解析完成: {content_id} - {parsed.title}")
    ...
    def _get_platform_cookies(self, platform: Platform) -> dict:
        ...
        if platform == Platform.BILIBILI:
            ...
            if settings.bilibili_bili_jct:
                cookies['bili_jct'] = settings.bilibili_bili_jct
            if settings.bilibili_buvid3:
                cookies['buvid3'] = settings.bilibili_buvid3
            return cookies
        return {}
