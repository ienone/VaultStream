...
启用此入口后，关闭对应管理员私聊及群聊原 NapCat HTTP `BotChat.is_monitoring`，保留订阅发送能力，避免配置误导。所有需要解析的群必须同时加入插件与后端白名单，不能只迁移一个群。后端 HTTP 入口已无条件忽略群消息，遗留或重新打开监控也不会恢复群聊自动收藏。群解析完成不触发收藏后的自动分发。
