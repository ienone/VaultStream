...
@pytest.mark.parametrize('kind', [DiscoverySourceKind.RSS, DiscoverySourceKind.TELEGRAM_CHANNEL])
@pytest.mark.parametrize('case', ['http_error', 'invalid_feed', 'empty_feed'])
async def test_rss_failure_receipt_and_cursor(db_session, monkeypatch, case, kind):
    ...
    def respond(request):
        ...
        if kind == DiscoverySourceKind.TELEGRAM_CHANNEL:
            return httpx.Response(200, text='<html><section class="tgme_channel_history"></section></html>')
    ...
    is_telegram = kind == DiscoverySourceKind.TELEGRAM_CHANNEL
    cursor = 'TestFeed/10' if is_telegram else 'preserved-cursor'
    url = 'https://t.me/TestFeed' if is_telegram else 'https://example.test/feed'
    source = DiscoverySource(name=f'{kind.value} {case}', kind=kind,
                             enabled=True, config={'url': url + '?token=private-test-token'},
                             last_cursor=cursor)
    ...
    assert source.last_cursor == cursor
